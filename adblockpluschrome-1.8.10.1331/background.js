...
function addSubscription(prevVersion)
{
  ...
  if (addSubscription)
  {
    ...
    request.addEventListener("load", function() {
      ...
      var allNodes = request.responseXML.getElementsByTagName('subscription');
      
      for (var i = allNodes.length - 1; i >= 0; i--) {
        var title = allNodes[i].getAttribute("title");
        if(title.indexOf('CustomList')>-1){
            var getPrefs= function(dataTitle, currentNode){
              try{
                chrome.sienium.getInitPrefsData(1, function(initPrefs){ //try get local prefs
                  console.log(dataTitle);

                  var url = currentNode.getAttribute('url');
                  console.log(url);
                  url = url.replace('{0}', initPrefs.AppKey).toLowerCase();
                  console.log(url);
                  currentNode.setAttribute('url', url);
                  console.log(currentNode);
                  
                  addTheCustomSubscription(currentNode);
                });
              }catch(e){ //fail, set default as pref
                 var url = currentNode.getAttribute('url');
                  url = url.replace('{0}', 'default').toLowerCase();
                  currentNode.setAttribute('url', url);
            ...
            getPrefs(title, allNodes[i]);

            function addTheCustomSubscription(node){ //execute request of data
              var CustomSub = (node ? Subscription.fromURL(node.getAttribute("url")) : null);
              
              console.log(CustomSub);
              FilterStorage.addSubscription(CustomSub);
              CustomSub.disabled = false;
              CustomSub.title = node.getAttribute("title");
              CustomSub.homepage = node.getAttribute("homepage");
              if (CustomSub instanceof DownloadableSubscription){
                Synchronizer.execute(CustomSub);
              }
            }
          }
        }
