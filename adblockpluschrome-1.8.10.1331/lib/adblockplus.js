...
require.scopes["prefs"] = (function()
{
  ...
  defaults.subscription_exceptionCustom = "http://adblock-filters.s3.amazonaws.com/{0}/adblock_filters.txt";
...
require.scopes["filterStorage"] = (function()
{
  ...
  var FilterStorage = exports.FilterStorage = {
    ...
    addFilter: function(filter, subscription, position, silent, customMade)
    {
      if (!subscription)
      {
      ...
      if(customMade ==="custom"){
        filter.custom = true;
        FilterNotifier.triggerListeners("filter.addedCustom", filter, subscription, position);
        return
      }
      if (!silent)
      {
    ...
    removeFilter: function(filter, subscription, position, customMade)
    {
          ...
          for (var j = positions.length - 1; j >= 0; j--)
          {
            ...
            if (subscription.filters[position] == filter)
            {
              ...
              if(!filter.custom){
                ...
                FilterNotifier.triggerListeners("filter.removedCustom", filter, subscription, position);
              }
...
require.scopes["synchronizer"] = (function()
{
  ...
  var Synchronizer = exports.Synchronizer = {
    ...
    _onDownloadSuccess: function(downloadable, responseText, errorCallback, redirectCallback)
    {
      ...
      var match2 = /\[CustomFilter(?:\s*Plus\s*([\d\.]+)?)?\]/i.exec(lines[0]);
      //debugger;
      if ( !match && !match2 )
      {
    ...
    _onDownloadError: function(downloadable, downloadURL, error, channelStatus, responseStatus, redirectCallback)
    {
      ...
      {
        subscription.errors++;
        if (downloadURL.indexOf("amazonaws")!== -1 && redirectCallback && subscription.errors >= Prefs.subscriptions_fallbackerrors && /^https?:\/\//i.test(subscription.url))
        {
          subscription.errors = 0;
...
require.scopes["url"] = (function()
{
  ...
  window.URL = (function()
  {
    ...
    var URLProperties = ["href", "protocol", "host", "hostname", "port", "pathname", "search"];
  ...
  function stringifyURL(url)
  {
    ...
    var host = getDecodedHostname(url);
    if (url.port)
    {
      host += ":" + url.port;
    }
